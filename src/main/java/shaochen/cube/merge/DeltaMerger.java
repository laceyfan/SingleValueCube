...
public class DeltaMerger {
	...
	private static void printHelp(Options options) {
		new HelpFormatter().printHelp("java -cp SingleValueCube.jar shaochen.cube.merge.DeltaMerger", options);
	}
	
	public static void main(String[] args) throws IOException {
		//解析命令行参数
		Options options = DeltaMerger.createCmdOptions();
		CommandLine cmd = null;
		try {
			cmd = new BasicParser().parse(options, args);
		} catch (ParseException e) {
			e.printStackTrace();
			DeltaMerger.printHelp(options);
			return;
		}
		...
		//配置Spark上下文
		String appName = (new StringBuilder("DeltaMerger")).append(" -i1 " + inputPath1).append(" -i2 " + inputPath2).toString();
